...
export default function RenderBreadcrumb({ currentPath, navigateToFolder }: FolderPath) {
  ...
  if (processedPath.length > 0 && processedPath[0].name.toLowerCase() === 'root') {
            ...
            <BreadcrumbItem>
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start">
                  {hiddenPath.map((item) => (
                    <DropdownMenuItem key={item.id} onClick={() => navigateToFolder(item.id, item.name)}>
                      {item.name}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
