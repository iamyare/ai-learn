...
const TranscriptionList: React.FC<TranscriptionListProps> = ({
  ...
  const getIndicesFromPosition = useCallback(
    (position: number) => {
      ...
      for (let i = 0; i < history.length; i++) {
        ...
        for (let j = 0; j < words.length; j++) {
          ...
          if (
            ...
            position < accumulatedLength + wordLength
          ) {
  ...
  useEffect(() => {
    const { paragraphIndex, wordIndex } =
      getIndicesFromPosition(currentPosition)
          ...
          <p
            className={cn(
              index === currentParagraphIndex
                ? 'bg-primary/5 p-1 rounded w-fit'
                : ''
            )}
          >
